...
public class Nachrichten extends Formular {
	...
	private static final List<Unterhaltung> UNTERHALTUNGEN = Arrays.asList(
			new Unterhaltung(1), new Unterhaltung(2));
	/**
	 * 
	 */
	private static final List<Nachricht> NACHRICHTEN = Arrays.asList(
			new Nachricht(), new Nachricht());
	...
	public void onLoad() {
		...
		hinzufuegenDialogBox
				.setText("Geben Sie einen Namen zum Hinzufügen eines Teilnehmers ein!");
		 ...
		 */
		final SuggestBox hinzufuegenSuggestBox = new SuggestBox(
				hinzufuegenOracle);
		...
		Button hinzufuegenButton = new Button("Hinzufügen", new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				// Hier Applikationslogik für hinzufügen des
				// Unterhaltungsteilnehmers einfügen!!!
			}
		});
		 ...
		 */
		Button nichtHinzufuegenButton = new Button("Abbrechen",
				new ClickHandler() {
					public void onClick(ClickEvent event) {
						hinzufuegenDialogBox.hide();
					}
		...
		loeschenDialogBox
				.setText("Geben Sie einen Namen zum Löschen eines Teilnehmers ein!");
		...
		/**
		 * SuggestBox, die anschließend dem Panel loeschenDialogBox hinzugefügt
		 * wird
		 */
		...
		Button loeschenButton = new Button("Löschen", new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				// Hier Applikationslogik für löschen des
				// Unterhaltungsteilnehmers !!!
			}
		});
		 ...
		 * Button, zum abbrechen
		 */
		Button nichtLoeschenButton = new Button("Abbrechen",
				new ClickHandler() {
					public void onClick(ClickEvent event) {
						loeschenDialogBox.hide();
					}
		...
		unterhaltungLoeschenDialogBox
				.setWidget(unterhaltungLoeschenDialogContents);

		/**
		 * Button, zum bestätigen des löschens der aktiven Unterhaltung
		 */
		Button unterhaltungLoeschenButton = new Button("Ja, löschen",
				new ClickHandler() {
					public void onClick(ClickEvent event) {
		 ...
		 */
		Button unterhaltungNichtLoeschenButton = new Button(
				"Nein, nicht löschen", new ClickHandler() {
					public void onClick(ClickEvent event) {
						unterhaltungLoeschenDialogBox.hide();
					}
				});
		...
		final Button neueNachrichtButton = new Button(
		...
		final Button unterhaltungDeleteButton = new Button(
		...
		CellList<Unterhaltung> UnterhaltungCellList = new CellList<Unterhaltung>(
				unterhaltungCell, UnterhaltungKeyProvider);
		...
		final SingleSelectionModel<Unterhaltung> UnterhaltungSelectionModel = new SingleSelectionModel<Unterhaltung>(
				UnterhaltungKeyProvider);
		...
		UnterhaltungSelectionModel
				.addSelectionChangeHandler(new SelectionChangeEvent.Handler() {
					public void onSelectionChange(SelectionChangeEvent event) {
						...
						// NutzerSelectionModel.getSelectedObject().xxx);
						// contactForm.setContact(selectionModel.getSelectedObject());
		...
		// ProvidesKey<Nachricht> NachrichtKeyProvider = new
		// ProvidesKey<Nachricht>() {
		...
		// Use the cell in a CellList.
		CellList<Nachricht> NachrichtCellList = new CellList<Nachricht>(
				nachrichtCell);
		...
		// // Add a selection model so we can select cells.
		// final SingleSelectionModel<Nachricht> NachrichtSelectionModel = new
		// SingleSelectionModel<Nachricht>(NachrichtKeyProvider);
		// NachrichtCellList.setSelectionModel(NachrichtSelectionModel);
		// NachrichtSelectionModel.addSelectionChangeHandler(new
		...
		/**
		 * Anlegen eines horizontalen Panels in dem man Hashtags hinzufügen kann
		 * und in dem die Hashtags angezeigt werden, die hinzugefügt wurden
		 */
		HorizontalPanel hashtagHinzufuegenPanel = new HorizontalPanel();
		unterhaltung.add(hashtagHinzufuegenPanel);

		/**
		 * Anlegen einer Textbox in der die hinzugefügten Hashtags angezeigt
		 * werden
		 */
		...
		/**
		 * Anlegen eines Buttons um Hashtags zu einer Nachricht hinzufügen zu
		 * können
		 */
